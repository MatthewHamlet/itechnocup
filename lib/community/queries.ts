...
import { clockOf, MONTHS_SHORT, calendarDayOf } from "./time";
...
function personOf(row: {
  ...
  const name = row.display_name?.trim() || "Warga Farad";
  return {
    id: row.id,
    name,
    role: row.headline?.trim() || "Warga",
...
async function peopleFor(ids: (string | null)[]): Promise<Map<string, CommunityPerson>> {
  ...
  const { data } = await supabase
    .from("profiles")
    .select("id, display_name, headline, verified, avatar_url")
    .in("id", unique);
  ...
  return data.map((row) => {
    ...
    return {
      ...
      author:
        ...
        personOf({ id: row.author_id as string, display_name: null, headline: null, verified: false }),
...
export const getCommunityGroups = cache(async (): Promise<CommunityGroup[]> => {
  ...
  return data.map((row) => ({
    ...
    tone: (row.tone as string) ?? "forest",
...
export const getSuggestedPeople = cache(async (limit = 4): Promise<CommunityPerson[]> => {
  ...
  return (profiles ?? [])
    ...
    .filter((row) => (row.display_name as string | null)?.trim())
    .slice(0, limit)
    .map(personOf);
});
  ...
  async (groupId: string, limit = 100): Promise<GroupMessage[]> => {
    ...
    return data
      .map((row) => {
        ...
        const name = person?.name ?? "Warga Farad";
        return {
          id: row.id as string,

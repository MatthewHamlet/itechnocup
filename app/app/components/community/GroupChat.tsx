    ...
    >
      {group && (
        <div className="flex flex-col">
          ...
          >
            <HealthPattern className="text-[#3f6a95]" opacity={0.13} />
                  ...
                  >
                    ...
                    <div
                      className={`max-w-[82%] rounded-2xl px-3 py-1.5 shadow-[0_1px_2px_rgba(24,32,24,0.06)] sm:max-w-[68%] ${
                        mine
                          ? `bg-farad-primary text-white ${head ? "rounded-tr-md" : ""}`
                          : `bg-white text-neutral-800 ring-1 ring-app-line ${head ? "rounded-tl-md" : ""}`
                      }`}
                    >
                      {head && !mine && (
                        <p className="text-[12px] font-bold text-farad-forest">{message.author}</p>
                      )}
          ...
          <form action={send} className="mt-3 flex items-end gap-2">
            ...
            <textarea
              ...
              className="min-h-[48px] flex-1 resize-none rounded-2xl bg-white px-4 py-3 text-[14.5px] leading-5 text-neutral-800 outline-none ring-1 ring-app-line placeholder:text-neutral-400 focus-visible:ring-2 focus-visible:ring-farad-primary/40 disabled:opacity-70"
            />
            <button
              type="submit"
              disabled={sending || !draft.trim()}
              aria-label="Kirim pesan"
              className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-farad-primary text-white outline-none transition-colors hover:bg-farad-forest focus-visible:ring-2 focus-visible:ring-farad-primary/40 disabled:opacity-50"
            >
          ...
          <button
            ...
            className="mt-3 inline-flex items-center gap-1.5 rounded-lg text-[12.5px] font-semibold text-neutral-500 outline-none transition-colors hover:text-farad-forest focus-visible:ring-2 focus-visible:ring-farad-primary/40"
          >
          ...
          {showMembers && (
            <ul className="mt-3 space-y-2 rounded-2xl bg-app-canvas/60 p-2.5 ring-1 ring-app-line">
              {members.map((member) => (
                <li key={member.id} className="flex items-center gap-3 rounded-xl bg-white px-3 py-2.5">
                  ...
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[14px] font-bold text-neutral-800">
                      {member.name}
                      {member.isAdmin && (
                        <span className="ml-2 rounded-full bg-farad-sage px-2 py-0.5 text-[11px] font-bold text-farad-forest">
                          Admin
                        </span>
                      )}
                    </span>

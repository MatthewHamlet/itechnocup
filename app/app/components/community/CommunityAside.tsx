...
import { count } from "@/lib/community/data";
        ...
        {topics.map((tag) => {
          ...
          return (
            <button
              ...
              className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-[12.5px] font-semibold outline-none ring-1 transition-transform duration-200 hover:-translate-y-0.5 focus-visible:ring-2 focus-visible:ring-farad-primary/40 ${
                on ? "bg-farad-primary text-white ring-farad-primary" : `${t.card} ${t.ring} ${t.ink}`
              }`}
            >
              {tag.label}
    ...
    <Panel title="Orang untuk Diikuti">
      ...
      {people.length === 0 && (
        <p className="py-6 text-center text-[13.5px] leading-5 text-neutral-500">
          Belum tersedia.
          <br />
          Rekomendasi muncul setelah ada pengguna Farad lain.
        </p>
      )}
          ...
          return (
            <li key={id} className="flex items-center gap-3">
              ...
              <button
                ...
                onClick={() => follow(id)}
                aria-pressed={isOn}
                className={`inline-flex shrink-0 items-center gap-1 rounded-full px-3 py-1.5 text-[12.5px] font-semibold outline-none transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-farad-primary/40 ${
                  isOn
                    ? "bg-farad-sage text-farad-forest ring-1 ring-app-line hover:bg-farad-sage/70"
